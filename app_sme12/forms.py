...
class RegistrationForm(forms.Form):
    ...
    ent_tumbol = forms.ModelChoiceField(label='ตำบล', queryset=Tumbol.objects.all(), empty_label='-- โปรดเลือก --', required=False)
    ...
    owner_tumbol = forms.ModelChoiceField(label='ตำบล', queryset=Tumbol.objects.all(), empty_label='-- โปรดเลือก --')
    ...
    business_model = forms.ModelChoiceField(label='รูปแบบการจัดตั้งกิจการ', queryset=BusinessModel.objects.all(), empty_label='-- โปรดเลือก --', required=False)
    ...
    product_info = forms.CharField(label='ระบุสินค้า ยี่ห้อ และบริการโดยละเอียด')
    material = forms.CharField(label='วัตถุดิบในการผลิต', required=False)
    ...
    f_train_or_contest = forms.CharField(label='เลือกอบรมหรือประกวด', max_length=100, required=False)
